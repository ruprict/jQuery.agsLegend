		...
		<li style="float:left">
			...
			{{if drawingInfo.renderer.type=="simple"}}
				...
				{{else drawingInfo.renderer.symbol.type=='esriSFS'}}
					<div style="display:table;padding:10px 0px 0px 45px;" >
					
						<div style="float:left;vertical-align:middle;width:25px;height:25px;background-color:${$item.getColor(drawingInfo.renderer.symbol.color)};border:${$item.getBorder(drawingInfo.renderer.symbol.outline)};"></div>
						<span style="padding:0px 5px 0px 5px;display:table-cell;vertical-align:middle;" >${name}</span>
					</div>
				...
				{{each drawingInfo.renderer.uniqueValueInfos}}
					...
					{{else $value.symbol.type=='esriSFS'}}
						<li style="margin:10px 5px;" >
							...
							<div style="float:left;display:table-cell;vertical-align:middle;width:25px;height:25px;background:${$item.getColor($value.symbol.color)};border:${$item.getBorder($value.symbol.outline)};" ></div>
					...
					{{else $value.symbol.type=='esriSLS'}}
						<li style="display:table;" >
							...
							<div style="float:left;margin:8px 5px 5px 5px;;width:25px;height:${$value.symbol.width}px;background:${$item.getColor($value.symbol.color)};"></div>
							<span style="padding:0px 5px 0px 5px;display:table-cell;vertical-align:middle;">${$value.label}</span>
						</li>
					{{/if}}
				{{/each}}
